-- ============================================================
-- PEAKS HOTEL — FULL SCHEMA (safe to run fresh)
-- Drop everything first, then recreate cleanly
-- ============================================================
...
-- user_roles
CREATE POLICY "roles read"          ON public.user_roles FOR SELECT TO authenticated USING (public.is_staff(auth.uid()) OR user_id = auth.uid());
...
-- bookings: staff full access + public insert (for website bookings)
...
CREATE POLICY "mpesa staff all" ON public.mpesa_transactions FOR ALL TO authenticated USING (public.is_staff(auth.uid())) WITH CHECK (public.is_staff(auth.uid()));
